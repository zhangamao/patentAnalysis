...
public class PublicNumberYearServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			JFreeChart jfreechart = ChartFactory.createLineChart("公开量年度分析",
					null, "公开量", dataset, PlotOrientation.VERTICAL, true,
					true, true);
			// 设置图表的子标题
	...
	public static CategoryDataset createDataset() throws SQLException {
		...
		pstmt.close();
		conn.close();
		int[] number = new int[publicYearList.size()];

		for (int k = 0; k < publicYearList.size(); k++) {
			number[k] = publicYearList.get(k);
			defaultDataset.addValue(number[k], "公开量", (2006 + k) + "年");
		}
